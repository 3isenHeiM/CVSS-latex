...
\begin{document}
    ...
    \section{Computation Tests}

    \cvssTest{}

    \section{Without CVSS string parsing}
    
    Con : have to split all the CVSS string in 8 arguments.
    
    Less re-usable in other projects
    
    \subsection{cvssBaseScorePretty}
    ...
    \section{With CVSS string parsing}
    
    \textbf{Goal}: 1 single argument, CVSS3 string
    
	\textbf{Available methods}
    \begin{itemize}
    	\item \texttt{computeCVSS}: only the CVSS score
    	\item \texttt{category}: the category of that score
    	\item \texttt{cvssTag}: color framed categroy of that score
    \end{itemize}
...
%	\edef\x{\computeCVSS{AV:L/AC:H/PR:N/UI:R/S:U/C:H/I:L/A:N}}
